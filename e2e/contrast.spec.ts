 ...
 *
 * dcyfr-app had no token-pair defect to fix — every `bg-primary` call site
 * here already names `text-primary-foreground`. The gate is ported anyway:
 * the pairing is correct today by authorship, not by enforcement, and the
 * fleet sweep that produced this commit found the same class of bug at four
 * sibling sites.
 *
